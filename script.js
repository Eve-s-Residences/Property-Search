 const properties = {
      ...
      "GREEN": {
        name: "GREEN",
        type: "Studio Unit",
        location: "Malinis St. Brgy. Sto Domingo Cainta, 1900 Rizal",
        map: "https://maps.app.goo.gl/ukNVYbjwcobdxfhMA",
        contracts: [
          "5,799 per month. Upper unit. 1 month advance, 2 months deposit",
          "5,399 per month. Lower unit. 1 year contract. 1 month advance, 2 months deposit."
        ],
        landmark: "Boundary of Pasig and Cainta. Walking distance to Ortigas Extension & Robinson's (Big R) Cainta Junction. Near Ortigas, Makati, Eastwood and Cubao.",
      },
      "ADI 168": {
    ...
    function highlightKeywords(text, keyword) {
      if (!text || !keyword || keyword === "all") return text;
      const regex = new RegExp(`(${keyword})`, "gi");
      return text.replace(regex, "<strong class='text-lime-600'>$1</strong>");
    }

    function containsAllWord(k) {
      if (!k) return false;
      return k.split(/\s+/).some(w => w === "all");
    }
    ...
    function search(locationKeyword, typeKeyword, landmarkKeyword) {
      ...
      const anyAll = containsAllWord(locationKeyword) || containsAllWord(typeKeyword) || containsAllWord(landmarkKeyword);
      const allFilled = locationKeyword && typeKeyword && landmarkKeyword;
      const showAll = anyAll || allFilled;

      for (const [_, data] of Object.entries(properties)) {
        const matchesLocation = showAll || locationKeyword === "" || data.location.toLowerCase().includes(locationKeyword);
        const matchesType = showAll || typeKeyword === "" || data.type.toLowerCase().includes(typeKeyword);
        const matchesLandmark = showAll || landmarkKeyword === "" || (data.landmark || "").toLowerCase().includes(landmarkKeyword);

        if (matchesLocation && matchesType && matchesLandmark) {
          matchFound = true;

          const table = document.createElement("table");
          table.className = "min-w-full bg-white text-sm text-left text-gray-600 shadow rounded overflow-hidden mb-8";
